...
class WNAFBlock_ffc3_2block(nn.Module):
    def __init__(self, c, num_heads=1, window_size=8, window_size_fft=-1, shift_size=-1, DW_Expand=2, FFN_Expand=2,
        ...
        if window_size_fft >= 0:
            self.fft_block1 = fft_bench_complex_mlp(c, DW_Expand, window_size=window_size_fft,
...
class WNAFBlock_ffc3_gelu_sin_2block_flops(nn.Module):
    def __init__(self, c, num_heads=1, window_size=8, window_size_fft=-1, shift_size=-1, DW_Expand=2, FFN_Expand=2,
        ...
        # self.sg = SimpleGate_frelu()
        if window_size_fft >= 0:
            self.fft_block1 = fft_bench_complex_mlp_flops(c, DW_Expand, window_size=window_size_fft, bias=True,
                                                          act_method=nn.GELU)  # , act_method=nn.GELU
            self.fft_block2 = fft_bench_complex_mlp_flops(c, DW_Expand, window_size=window_size_fft, bias=True,
                                                          act_method=nn.GELU)
        ffn_channel = FFN_Expand * c
        self.conv4 = nn.Conv2d(in_channels=c, out_channels=ffn_channel, kernel_size=1, padding=0, stride=1, groups=1,
                               bias=True)
...
class FNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1],
                 window_size_e=[64, 32, 16, 8], window_size_m=[8], window_size_e_fft=[64, 32, 16, -1],
                 window_size_m_fft=[-1],
        ...
        for i in range(len(enc_blk_nums)):
            self.encoders.append(
                nn.Sequential(
                    *[NAFBlock(chan, num_heads_e[i], window_size_e[i], window_size_e_fft[i], window_sizex_e[i]) for _ in
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFBlock(chan, num_heads_m[0], window_size_m[0], window_size_m_fft[0], window_sizex_m[0]) for _ in
        ...
        for j in range(len(dec_blk_nums)):
            ...
            self.decoders.append(
                nn.Sequential(
                    *[NAFBlock(chan, num_heads_d[j], window_size_d[j], window_size_d_fft[j], window_sizex_d[j]) for _ in
